...
(function($) {
  ...
  function add_clear_button(input, set_focus) {
    if (input.attr('has_clearable_button')!="1") {
      ...
      /*  when this code is in place for some reason there are focus
       *  problems in IE.  Fortunately it doesn't seem to do anything useful
       *  so it is commented out - SMG
      ...
      */
      ...
      clear_button.click(function(){
        input.val('');
        $(this).parent().find("input").trigger("textFieldCleared")
        trigger(input);
        input.change();
      });
