...
(function() {
  ...
  contextPrototype.drawImage = function(image) {
    ...
    // XXXSMG: AlphaImageLoader does this awesome thing where it decodes the
    // image.src URL for no apparent reason.  So we have to hack around
    // that by adding a 'doubleSrc' member to the image object, which
    // contains a version of the URL which is double encoded. 
    if( (image.doubleSrc !== null) && (image.doubleSrc !== undefined) ) {
        vmlStr.push(' progid:DXImageTransform.Microsoft.AlphaImageLoader(src=', image.doubleSrc, ',sizingMethod=scale)">');
    }
    else {
